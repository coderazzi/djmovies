...
MIN_AGE_IN_SECONDS_TO_REQUERY=86400*2 #2 days
...
def _get_results(query_id, exclude_status=5000):
    return UResults.objects.filter(query_id=query_id).exclude(status=exclude_status).extra(select={'ct_day':'creation_time/86400'}).order_by('-ct_day', '-oid')
...
def _update_results(query, results=[], only_get_newer_results=True):
	...
	for d in reversed(new_results):
		UResults(query_id=query.id, oid=d.oid, desc=d.desc, size=d.size, nfo=d.nfo, 
			...
			status=STATUS_NO_DOWNLOADED, creation_time=now).save()
